 ...
 integer, parameter :: maxyears=2400
 ...
 character(len=12) :: head(10,24),numPFTs
 ...
 real,   dimension(maxyears,10):: siteplantC, sitesoilC,siteplantN,siteSoilN,siteMineralN,siteLAI &
                                , siteTransp, siteEvap, siteRunoff
 ...
 integer :: totyears,m,n,i,j,k,iPFT,iLayer,layertype, itag
 ...
 ! Input files
   filepath = 'rerun0521/SC0.1/AllVarRL/' ! ALLfixedRL/' ! 'SC01ML08/' ! 'SC01ML07/' !'SC0.1ML0.6/' ! 'NCAPS0.1/' ! 'r_N_SD/'
   ! AllVarRL/' !ALLfixedRL/' ! 'rerun1120/FixedRL8PFTs/' !
   SOMlevels = 8
   initialPFTs =  8 ! 8 ! 1 !
   siteplus    =  9 ! 9 ! 7 !
...
do itag = 1, 2
   ...
   do iSOM = 1, 8
      ...
      do iTests =1, totTests
         ...
         fileout2 = trim(filepath)//'Sum_'//trim(chaSOM(iSOM))//trim(runtag)//trim(parameterfile(iTests))//'.csv'
         ! Open input files
...
!             size classes distribution, including total individuals; biomass distribution
              ...
              if(dataarray(7,j,m)> 0.0)then ! DBH > 0
                  ...
                  ! for growth: Unit *1000/10000 = gC per m2
...
! read in another file for combining data
    ...
    /   'BA1','BA2','BA3','BA4', 'BA5','BA6','BA7','BA8','BA9','BA10',                               &    ! 1
        ...
        'LAI1','LAI2','LAI3','LAI4', 'LAI5','LAI6','LAI7','LAI8','LAI9','LAI10',                     & ! 21
        'tsp1','tsp2','tsp3','tsp4', 'tsp5','tsp6','tsp7','tsp8','tsp9','tsp10',                      & ! 22
        'evp1','evp2','evp3','evp4', 'evp5','evp6','evp7','evp8','evp9','evp10',           & ! 23
        'ruf1','ruf2','ruf3','ruf4', 'ruf5','ruf6','ruf7','ruf8','ruf9','ruf10'                      & ! 24
    /
 ...
 do m = 1, totyears
     ...
     siteMineralN(m,iTests) = ecodata(34,m) !mineralN pool; ecodata(37,m), yearly rate
     siteTransp(m,iTests)   = ecodata(8,m)
     siteEvap(m,iTests)     = ecodata(9,m)
     siteRunoff(m,iTests)   = ecodata(10,m)
     siteLAI(m,iTests)      = ecodata(2,m)
 ...
 enddo
  ...
  write(155,'(960(a12,","))') 'year',             &      ! Single PFT model runs
        ...
        (head(iTests,21),iTests=1,siteplus-initialPFTs), & !  LAI
        (head(iTests,22),iTests=1,siteplus-initialPFTs), & !  Transpiration
        (head(iTests,23),iTests=1,siteplus-initialPFTs), & !  Evaporation
        (head(iTests,24),iTests=1,siteplus-initialPFTs)    !  Runoff

          do m = 1, totyears
              write(155,140)m, &
                    (siteBA(m,iTests),       iTests=1,siteplus-initialPFTs),   & ! Single PFT model runs
                    ...
                    (siteLAI(m,iTests),      iTests=1,siteplus-initialPFTs),   &
                    (siteTransp(m,iTests),   iTests=1,siteplus-initialPFTs),   &
                    (siteEvap(m,iTests),     iTests=1,siteplus-initialPFTs),   &
                    (siteRunoff(m,iTests),   iTests=1,siteplus-initialPFTs)
